...
api.ajaxRequest = function (route, data, callback) {
  if (arguments.length === 2) {
    callback = arguments[1];
    data = {};
  }
  ...
  this.options.debug('api:request:' + route, data);
  this.options.retrieveToken(_.bind(function (err, token) {
    if (err) {
      return this.options.debug('api:token:' + route + ':error', response);
    }
    $.ajax({
      url: '/api/' + route,
      ...
      error: _.bind(function (jqXHR, textStatus, errorThrown) {
        this.options.debug('api:response:' + route + ':transport:error', textStatus, errorThrown);
      }, this),
      success: _.bind(function (response, textStatus, jqXHR) {
        if (response.err) {
          this.options.debug('api:response:' + route + ':error', response);
        } else {
          this.options.debug('api:response:' + route, response);
        }
        return callback(response);
      }, this)
    });
...
api.home = function (limit, callback) {
  this.ajaxRequest('home', {
    limit: limit
  }, callback);
};
api.search = function (options, callback) {
  this.ajaxRequest('search', options, callback);
};
api.signalUser = function (userId, inRoomId, category, reason, callback) {
  var data = {
    user_id: userId,
    room_id: inRoomId,
    category: category,
  ...
  this.pomeloRequest('chat.signalHandler.user', data, callback);
};
api.signalEvent = function (eventId, category, reason, callback) {
  var data = {
    event: eventId,
    category: category,
    reason: reason
  };
  this.pomeloRequest('chat.signalHandler.event', data, callback);
};
...
api.groupRequestEmail = function (groupId, email, callback) {
  var data = {
    group_id: groupId,
    email: email
  };
  this.pomeloRequest('chat.groupRequestEmailHandler.call', data, callback);
};
api.groupQuitMembership = function (groupId, callback) {
  var data = {group_id: groupId};
  this.pomeloRequest('chat.groupQuitMembershipHandler.call', data, callback);
};
...
api.groupBecomeMember = function (groupId, password, callback) {
  var data = {group_id: groupId, password: password};
  this.pomeloRequest('chat.groupBecomeMemberHandler.call', data, callback);
};
...
api.groupJoin = function (groupId, callback) {
  if (!groupId) {
    return;
  }
  var data = {
...
api.roomInvite = function (roomId, userId, callback) {
  var data = {room_id: roomId, user_id: userId};
  this.pomeloRequest('chat.roomInviteHandler.call', data, callback);
};
api.roomRefuse = function (roomId, userId, callback) {
  var data = {room_id: roomId, user_id: userId};
  this.pomeloRequest('chat.roomRefuseRequestHandler.call', data, callback);
};
api.roomAccept = function (roomId, userId, callback) {
  var data = {room_id: roomId, user_id: userId};
  this.pomeloRequest('chat.roomAcceptRequestHandler.call', data, callback);
};
...
api.roomBecomeMember = function (roomId, callback) {
  var data = {room_id: roomId};
  this.pomeloRequest('chat.roomBecomeMemberHandler.call', data, callback);
};
...
api.roomHistory = function (roomId, id, direction, limit, callback) {
  this._history({
    room_id: roomId,
    id: id,
    direction: direction,
...
api.userHistory = function (userId, id, direction, limit, callback) {
  this._history({
    user_id: userId,
    id: id,
    direction: direction,
...
api._history = function (data, callback) {
  this.ajaxRequest('history', data, callback);
};
api.getRoomEvent = function (roomId, eventId, callback) {
  this.pomeloRequest('history.getRoomEventHandler.call', {
    room_id: roomId,
    event: eventId
  }, callback);
};
api.getUserEvent = function (userId, eventId, callback) {
  this.pomeloRequest('history.getUserEventHandler.call', {
    user_id: userId,
    event: eventId
  }, callback);
};
...
api.notificationDone = function (ids, all) {
  var data = {ids: ids, all: all};
