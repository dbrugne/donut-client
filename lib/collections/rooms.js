...
module.exports = Backbone.Collection.extend({
  ...
  onUnblocked: function (data) {
    ...
    this.app.client.roomJoin(model.get('id'), null, _.bind(function (response) {
      if (response.err) {
        this.app.trigger('error', response.code, response.err); // not listened yet
      }
    }, this));
  },
  ...
  onLeave: function (data) {
    ...
    if (data.reason && data.reason === 'deleted' && model.get('group_id')) {
      var model = this.app.groups.get(model.get('group_id'));
      if (model) {
        model.onRefresh();
      }
    }
